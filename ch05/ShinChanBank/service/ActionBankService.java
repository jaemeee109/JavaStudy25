...
public class ActionBankService {


	String actionMasterid = "acmaster";
	String actionMasterpw = "5678";


		public void menu(Scanner inputInt, Scanner inputStr, AccountDTO[] ActionBank, AccountDTO[] kasukabeBank) {
			
			boolean subRun = true;
			while (subRun) {
				System.out.println("↓↓↓ 액션은행 메뉴 ↓↓↓");
				System.out.println("---------------------------");
				...
				case "1" :
					...
					System.out.println("---------------------------");
					CreateAccount(inputInt, inputStr, ActionBank);
					System.out.println("---------------------------");
					break;
				case "2" : // 직원용
					System.out.println("☆ 계좌 목록 ☆");
					System.out.println("---------------------------");
					AccountList(inputInt, inputStr, ActionBank);
					System.out.println("---------------------------");
					break;
				case "3" :
					System.out.println("☆ 입금하기 ☆");
					System.out.println("---------------------------");
					deposit (inputInt, inputStr, ActionBank);
					System.out.println("---------------------------");
					break;
				case "4" :
					System.out.println("☆ 출금하기 ☆");
					System.out.println("---------------------------");
					withdraw (inputInt, inputStr, ActionBank);
					System.out.println("---------------------------");
					break;
				case "5" :
		...
		private void withdraw(Scanner inputInt, Scanner inputStr, AccountDTO[] ActionBank) {
			// 출금 메서드
			...
			int money = inputInt.nextInt();
				...
				AccountDTO inputAccount = findAccount (ano, ActionBank, ActionBank);
				// find Account 메서드는 배열에서 객체를 찾고, 찾은 객체를 리턴하는 역할을 함 (반복코드 배제용)
		...
		private void deposit(Scanner inputInt, Scanner inputStr, AccountDTO[] ActionBank) {
			// 입금 메서드
		...
		private AccountDTO findAccount(String ano, AccountDTO[] ActionBank, AccountDTO[] kasukabeBank) {
		...
		private void AccountList(Scanner inputInt, Scanner inputStr, AccountDTO[] ActionBank) {
			// 액션은행 직원용 계좌목록 관리 메뉴
			...
			if (id.equals(actionMasterid) && pw.equals(actionMasterpw)) {
				
				System.out.println("↓↓↓ 떡잎마을 계좌 리스트 ↓↓↓");
				System.out.println("---------------------------");
				for (int i = 0; i< ActionBank.length; i++) {
					AccountDTO accountList = ActionBank[i];
					if (accountList != null) {
		...
		private void CreateAccount(Scanner inputInt, Scanner inputStr, AccountDTO[] ActionBank) {
			// 액션은행 계좌생성
			...
			myAccount.setBankName("액션"); // 빈객체에 데이터 입력 완료
			
			// 떡잎은행 배열에 객체를 삽입 (null인지 확인하고 넣을것)
			for (int i = 0; i<ActionBank.length; i++) {
				
				if (ActionBank[i] == null) {
					//떡잎은행 배열에 null 값이 있으면 처리하는 코드
					ActionBank[i] = myAccount; // 배열에 입력한 객체가null에 삽입된다
				//	System.out.println("계좌 입력 완료 테스트: "+ActionBankBank[i]);
					break;
				}//if종료
			...
			System.out.println(" ☆ 액션은행 계좌가 성공적으로 만들어졌습니다 ☆ ");
